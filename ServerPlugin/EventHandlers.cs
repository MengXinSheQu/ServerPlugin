...
namespace ServerPlugin
{
    public class EventHandlers
    {
        ...
        { 
            if(Plugin.Ins.Config.NoFog)
                ev.Player.EnableEffect(EffectType.FogControl);
        }
        private void OnInteractingScp330(InteractingScp330EventArgs Args)
        {
            if (Args.UsageCount < Plugin.Ins.Config.Candys)
                Args.ShouldSever = false;
        }
        private void OnHurting(HurtingEventArgs Args)
        {
            if (Args.DamageHandler.Type != DamageType.Scp207 && Plugin.Ins.Config.DisableSCP207Hurt)
                Args.Amount = 0f;
            if (Args.DamageHandler.Type != DamageType.Poison && Plugin.Ins.Config.DisablePoison)
                Args.Amount = 0f;
        }
        private void OnDroppingAmmo(DroppingAmmoEventArgs Args)
        {
            if (Plugin.Ins.Config.InfAmmo)
                Args.IsAllowed = false;
        }
        private void OnDroppingItem(DroppingItemEventArgs Args)
        {
            if (!Args.Item.IsAmmo && Plugin.Ins.Config.InfAmmo)
                Args.IsAllowed = false;
        }
        ...
        {
            if (!Plugin.Ins.Config.RemoteKeycard || BlacklistPlayers.Contains(ev.Player))
                return;
            if (ev.Player.IsScp && Plugin.Ins.Config.RemoteKeycard_Scp)
                return;
